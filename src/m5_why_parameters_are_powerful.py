...
def main():
    ...
    even_better_draw_circles(rg.Point(100, 50),3,4,'Green',2)
    even_better_draw_circles(rg.Point(-200, 0),15,6,'Red',10)
...
def better_draw_circles(point,incamt):
    """
    ...
    while k<(incamt*10):
        ...
        # Go DOWN 15 pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(incamt)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(k)  # Radius 1
        k+=incamt
...
def even_better_draw_circles(point,incamt,number,color,thickness):
    """
    Starts out the same as the   draw_circles   function defined ABOVE.
    You Will make it an IMPROVED, MORE POWERFUL function per the above _TODO_.
    """
